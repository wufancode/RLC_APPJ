...
class PPO:
    ...
    def build_actor_net(self, name, trainable):
        with tf.compat.v1.variable_scope(name):
            ...
            mu = (tf.compat.v1.layers.dense(l2, self.action_dim, tf.compat.v1.nn.relu, trainable=trainable) + 1) / 2
...
class Jet(object):
    def __init__(self):
        self.target_density1 = np.array([[0]], dtype='float32')
        self.output_density1 = np.array([[0] * hist_num], dtype='float32')
        self.target_density2 = np.array([[0]], dtype='float32')
        self.output_density2 = np.array([[0] * hist_num], dtype='float32')

        self.distance1 = self.target_density1 - self.output_density1
        self.distance2 = self.target_density2 - self.output_density2

        self.action = np.array([[0] * hist_num * act_num], dtype='float32')
        self.state = np.concatenate((self.distance1, self.distance2, self.action), axis=1)
        ...
        state_low = np.array([0] * hist_num * (2 + act_num))
        state_high = np.array([1] * hist_num * (2 + act_num))
    ...
    def step(self, action, vol=1, flow=0.5, inter=0.33, p_o2=0, p_h2o=0, fre=1):
        ...
        self.output_density1 = np.roll(self.output_density1, 1)
        self.output_density1[0][0] = density_no
        self.output_density2 = np.roll(self.output_density2, 1)
        self.output_density2[0][0] = density_no
        ...
        self.distance2 = self.target_density2 - self.output_density2
        self.state = np.concatenate((self.distance1, self.distance2, self.action), axis=1)
        dis1 = abs(density_no - self.target_density1).sum()
        dis2 = abs(density_o - self.target_density2).sum()
        reward = -dis1-dis2
        # reward = reward[0, 0]
        if dis1 < 0.05 and dis1 < 0.05:
            reward = 20
        elif dis1 < 0.1 and dis1 < 0.1:
            reward = 10
...
def main():
    ...
    target = np.random.uniform(0, 0.8, (ROLL_STEPS, 2))
    if train == 1:
        for episode in range(EPISODES):
            agent.resetMemory()
            ep_r = 0
            for roll in range(ROLL_STEPS):
                env.target_density1 = np.array([target[roll, 0]])
                env.target_density2 = np.array([target[roll, 1]])
                state = env.reset()
        ...
        for T1 in test_seq:
            # output the action
            env.target_density2 = T1
            env.target_density1 = 0.3
            action = agent.Choose_Action(state)
            # process the action and get the info
            next_state, reward, done, info = env.step(action)
            # record the reward
            out_v1 = np.append(out_v1, env.target_density1 - state[0][0])
            out_v2 = np.append(out_v2, env.target_density2 - state[0][2])
            act_list = np.append(act_list, action)
            state = next_state
        fig, ax = plt.subplots()
        x = np.linspace(1, 360, 360)
        ax.plot(x, np.array(out_v1), label='out1')
        ax.plot(x, np.array(test_seq), label='set1')
        ax.plot(x, np.array(out_v2), label='out2')
        ax.plot(x, np.array([0.3]*360), label='set2')
        ax.set_xlabel("Step")
